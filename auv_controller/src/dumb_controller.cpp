...
public:
  ...
  : Node("dumb_controller")
  {
    ...
    twist_subscription_ = this->create_subscription<geometry_msgs::msg::Twist>(
      "cmd_vel", 10,
      std::bind(
        &DumbController::twist_callback, this,
        std::placeholders::_1));  // for setting target velocities for intelligence

    current_depth_sub_ = this->create_subscription<std_msgs::msg::Float64>(
      "/current_depth", 10,
      std::bind(
        &DumbController::current_depth_callback, this,
        std::placeholders::_1));  // for getting current depth

    target_depth_sub_ = this->create_subscription<std_msgs::msg::Float32>(
      "/target_depth", 10,
      std::bind(
        &DumbController::target_depth_callback, this,
        std::placeholders::_1));  // for getting target depth
    ...
    publish_timer_ = this->create_wall_timer(
      std::chrono::milliseconds(publish_rate), std::bind(&DumbController::timer_callback, this));
  }
...
private:
  ...
  void control_callback()
  {
    ...
    if (!depth_reached_ && std::abs(depth_error) <= threshold) {
      ...
      RCLCPP_INFO(
        this->get_logger(),
        "Depth control - Current: %.2f, Target: %.2f, Error: %.2f, Command: %.2f", current_depth_,
        target_depth_, depth_error, target_vel_heave);
    }
    ...
    heave_pwm_ = calculatePWM(
      target_vel_heave, heave_high_cut_off, heave_medium_cut_off, heave_low_cut_off,
      heave_high_pwm_change_, heave_medium_pwm_change_, heave_low_pwm_change_);

    yaw_pwm_ = calculatePWM(
      target_vel_yaw, yaw_high_cut_off, yaw_medium_cut_off, yaw_low_cut_off, yaw_high_pwm_change_,
      yaw_medium_pwm_change_, yaw_low_pwm_change_);

    surge_pwm_ = calculatePWM(
      target_vel_surge, surge_high_cut_off, surge_medium_cut_off, surge_low_cut_off,
      surge_high_pwm_change_, surge_medium_pwm_change_, surge_low_pwm_change_);

    RCLCPP_INFO(
      this->get_logger(),
      "\n\tSurge | Yaw | Heave\nTarget_vel: [%.2f, %.2f, %.2f]\nPWM sent to thrusters: [%d, %d,%d]",
      target_vel_surge, target_vel_yaw, target_vel_heave, surge_pwm_, yaw_pwm_, heave_pwm_);
  }
  // TODO add custmized PWM for backward motion
  int calculatePWM(
    float target_vel, const float high_cut_off, const float medium_cut_off, const float low_cut_off,
  ...
  void publish_rc_override()
  {
    ...
    RCLCPP_DEBUG(
      this->get_logger(), "Published RC Override: Surge: %d, Yaw: %d, Heave: %d", surge_pwm_,
      yaw_pwm_, heave_pwm_);
  }
