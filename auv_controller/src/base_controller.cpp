...
public:
  ...
  KalmanFilter(
    double process_noise, double measurement_noise, double estimated_error, double initial_value)
...
public:
    ...
    kalman_heave(0.01, 0.5, 1.0, 0.0)
  {
    ...
    kalman_surge =
      KalmanFilter(k_process_noise, k_measurement_noise, k_estimated_error, k_initial_value);
    kalman_yaw =
      KalmanFilter(k_process_noise, k_measurement_noise, k_estimated_error, k_initial_value);
    kalman_heave =
      KalmanFilter(k_process_noise, k_measurement_noise, k_estimated_error, k_initial_value);
...
private:
  ...
  void imu_callback(const sensor_msgs::msg::Imu & msg)
  {
    ...
    surge_pwm_ = compute_motor_pwm(
      target_vel_surge, velocity_surge, error_surge, integral_surge, previous_error_surge, dt);
    yaw_pwm_ = compute_motor_pwm(
      target_vel_yaw, velocity_yaw, error_yaw, integral_yaw, previous_error_yaw, dt);
    heave_pwm_ = compute_motor_pwm(
      target_vel_heave, velocity_heave, error_heave, integral_heave, previous_error_heave, dt);
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "\nTarget_vel: [%.2f, %.2f, %.2f]\nMeasured_vel: [%.2f, %.2f,%.2f]\nPWM sent to thrusters: "
      "[%d, %d,%d]",
      target_vel_surge, target_vel_yaw, target_vel_heave, velocity_surge, velocity_yaw,
      velocity_heave, surge_pwm_, yaw_pwm_, heave_pwm_);
  ...
  int compute_motor_pwm(
    double target_vel, double current_vel, double & error, double & integral,
    double & previous_error, double dt)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "PID Debug: Error: %.2f, Integral: %.2f, Derivative: %.2f, PWM Adjustment: %d", error,
      integral, derivative, pwm_adjustment);
