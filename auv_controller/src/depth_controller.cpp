...
public:
  DepthControl() : Node("depth_controller"), depth_reached_(false)
  {
    ...
    current_depth_sub_1 = this->create_subscription<std_msgs::msg::Float64>(
      "/current_depth", 10,
      std::bind(
        &DepthControl::current_depth_callback1, this,
        std::placeholders::_1));  // wrong topic, change later.
...
private:
  ...
  void control_loop()
  {
    ...
    if (std::abs(depth_error) <= threshold) {
      RCLCPP_INFO(
        this->get_logger(), "Target depth %.2f reached. Turning OFF depth control", target_depth_);
      ...
      RCLCPP_DEBUG(
        ...
        target_depth_, depth_error, cmd_vel.linear.z);
    }
