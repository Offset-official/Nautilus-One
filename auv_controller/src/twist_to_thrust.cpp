...
public:
  TwistToThrust()
  : Node("twist_to_thrust"), neutral_pwm(1500), pwm_range(50)  // PWM range for [-1.0, 1.0]
  {
    ...
    publish_timer_ =
      this->create_wall_timer(100ms, std::bind(&TwistToThrust::publish_rc_override, this));
  }
...
private:
  void topic_callback(const geometry_msgs::msg::Twist & msg)
  {
    ...
    RCLCPP_INFO(
      this->get_logger(),
      "Received cmd_vel: Heave: %.2f, Forward: %.2f, Yaw: %.2f -> PWM: %d, %d, %d", msg.linear.z,
      msg.linear.x, msg.angular.z, heave_pwm_, surge_pwm, yaw_pwm_);
  }
  ...
  void publish_rc_override()
  {
    ...
    RCLCPP_DEBUG(
      this->get_logger(), "Published RC Override: Heave: %d, Forward: %d, Yaw: %d", heave_pwm_,
      surge_pwm, yaw_pwm_);
  }
