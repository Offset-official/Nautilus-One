...
private:
  void imageCallback(const sensor_msgs::msg::CompressedImage::SharedPtr msg) const
  {
    try {
      ...
      cv::Mat compressed_data(1, static_cast<int>(msg->data.size()), CV_8UC1, (void*)msg->data.data());

      // Decode the compressed image to a color (BGR) Mat
      cv::Mat frame = cv::imdecode(compressed_data, cv::IMREAD_COLOR);

      if (frame.empty()) {
        RCLCPP_WARN(this->get_logger(), "Decoded image is empty. Check if the compressed data is valid.");
        return;
      }
