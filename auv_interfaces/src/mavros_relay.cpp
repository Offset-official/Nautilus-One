...
public:
  MavrosRelay() : Node("mavros_relay"), armed_(false)
  {
    ...
    state_sub_ = this->create_subscription<mavros_msgs::msg::State>(
      "/mavros/state", 10, std::bind(&MavrosRelay::stateCallback, this, std::placeholders::_1));

    // Publisher for the new topic
    diagnostics_pub_ =
      this->create_publisher<auv_interfaces::msg::Diagnostics>("/auv_interfaces/relay", 10);

    // Timer to publish messages
    timer_ = this->create_wall_timer(
      std::chrono::seconds(2), std::bind(&MavrosRelay::publishDiagnostics, this));
  }
...
private:
  ...
  void publishDiagnostics()
  {
    ...
    RCLCPP_INFO(this->get_logger(), "Published Mediator Data: armed=%d", msg.armed);
  }
