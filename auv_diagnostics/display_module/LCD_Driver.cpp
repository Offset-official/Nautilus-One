...
void LCD_SetWindow(UWORD Xstart, UWORD Ystart, UWORD Xend, UWORD Yend)
{
  ...
  LCD_WriteData_Byte(0x00);
  LCD_WriteData_Byte((LCD_WIDTH - Xend) & 0xff);  // Reverse X coordinate
  LCD_WriteData_Byte((LCD_WIDTH - Xstart - 1) >> 8);
  LCD_WriteData_Byte((LCD_WIDTH - Xstart - 1) & 0xff);
...
void LCD_SetCursor(UWORD X, UWORD Y)
{
  ...
  LCD_Write_Command(0x2b);
  LCD_WriteData_Byte((LCD_WIDTH - X) >> 8);  // Reverse X coordinate
  LCD_WriteData_Byte(LCD_WIDTH - X);
  LCD_WriteData_Byte((LCD_WIDTH - X) >> 8);
  LCD_WriteData_Byte(LCD_WIDTH - X);
...
{
  unsigned int i, j;
  LCD_SetWindow(0, 0, LCD_HEIGHT, LCD_WIDTH);  // Swap WIDTH and HEIGHT
  DEV_Digital_Write(DEV_DC_PIN, 1);
  for (i = 0; i < LCD_HEIGHT; i++) {  // Swap loop bounds
