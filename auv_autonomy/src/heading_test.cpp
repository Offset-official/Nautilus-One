...
private:
    void execute_sequence()
    {
        ...
        // Wait for depth action to complete, then continue with the sequence
        auto wait_timer = this->create_wall_timer(
            5s, 
            [this]() {
                // Cancel this timer
                wait_timer_->cancel();
                
                // Enable angle correction if configured
                ...
                RCLCPP_INFO(this->get_logger(), "Moving forward with speed: %f", linear_speed_);
                
                // Publish velocity command continuously for 10 seconds
                ...
                // Create a timer to stop movement after 10 seconds
                stop_timer_ = this->create_wall_timer(
                    ...
                    [this]() {
                        ...
                        RCLCPP_INFO(this->get_logger(), "Stop moving forward.");
        ...
        wait_timer_ = wait_timer; // Store the timer to prevent it from going out of scope
    }
