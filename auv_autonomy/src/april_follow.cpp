...
public:
  ...
  : Node("april_follow_node")
  {
      ...
      [this](const std::vector<rclcpp::Parameter> & params) {
        ...
        for (auto & param : params) {
          if (param.get_name() == "follow_id") {
            ...
            RCLCPP_INFO(
              this->get_logger(),
              "Parameter 'follow_id' changed to %d. Waiting for that tag to appear...", follow_id_);
          } else if (param.get_name() == "forward_speed") {
            forward_speed_ = param.as_double();
            RCLCPP_INFO(
              this->get_logger(), "Parameter 'forward_speed' changed to %.3f.", forward_speed_);
          } else if (param.get_name() == "kp_yaw") {
            kp_yaw_ = param.as_double();
            RCLCPP_INFO(this->get_logger(), "Parameter 'kp_yaw' changed to %.3f.", kp_yaw_);
          }
        }
        return result;
      });
    ...
    timer_ = this->create_wall_timer(400ms, std::bind(&AprilFollowNode::controlLoop, this));
    ...
    RCLCPP_INFO(
      this->get_logger(), "Initialization done. Will follow AprilTag ID = %d once detected...",
      follow_id_);
  }
  ...
  ~AprilFollowNode() override
  {
    ...
    RCLCPP_INFO(this->get_logger(), "AprilFollowNode shutting down...publishing zero velocity.");
...
private:
  ...
  // -------------------------------------------------------
  enum class FollowState { WAITING_FOR_TAG, FOLLOWING_TAG };
  ...
  void detectionsCallback(const apriltag_msgs::msg::AprilTagDetectionArray::SharedPtr msg)
  {
    ...
    for (auto & detection : msg->detections) {
      ...
      if (detection.id == follow_id_) {
        ...
        april_center_ = std::make_pair(static_cast<int>(center_x), static_cast<int>(center_y));
    ...
    if (found_tag && state_ != FollowState::FOLLOWING_TAG) {
      RCLCPP_INFO(
        this->get_logger(), "Found AprilTag ID=%d => switching to FOLLOWING_TAG.", follow_id_);
  ...
  geometry_msgs::msg::Twist doAprilTagAlignment()
  {
    ...
    RCLCPP_INFO(
      this->get_logger(), "->Tag alignment => err_x=%.1f => forward=%.2f, yaw=%.4f", error_x,
