...
public:
  ...
  using LEDColor = auv_interfaces::srv::SetColor;
  ...
  explicit ReadCommSequenceActionServer(
    ...
    this->led_color_client_ = this->create_client<LEDColor>("/set_color");
...
private:
  ...
  rclcpp::Client<LEDColor>::SharedPtr led_color_client_;
  image_transport::Subscriber image_sub_;
  sensor_msgs::msg::Image::ConstSharedPtr img_;

  std::string color_codes[3] = {"#fff0000", "#00ff00", "#0000ff"};
  ...
  void execute(const std::shared_ptr<GoalHandleReadCommSequence> goal_handle) {
    ...
    while (sequence.size() != 3) {
      ...
      if ((idx != -1) && (sequence.size() == 0 || idx != sequence.back())) {
        ...
        // set the desired LED color to reflect the acknowledgement
        auto color_request = std::make_shared<LEDColor::Request>();
        color_request->color = color_codes[idx];
        auto _color_res = led_color_client_->async_send_request(color_request);
        _color_res.wait();
