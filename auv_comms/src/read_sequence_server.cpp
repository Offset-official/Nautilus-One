...
public:
  ...
  explicit ReadCommSequenceActionServer(const rclcpp::NodeOptions & options = rclcpp::NodeOptions())
  : Node("read_comm_sequence_server", options)
  {
    using namespace std::placeholders;

    this->action_server_ = rclcpp_action::create_server<ReadCommSequence>(
      this, "read_comm_sequence", std::bind(&ReadCommSequenceActionServer::handle_goal, this, _1),
      std::bind(&ReadCommSequenceActionServer::handle_cancel, this, _1),
      std::bind(&ReadCommSequenceActionServer::handle_accepted, this, _1));

    this->color_detect_client_ = this->create_client<ImageColorDetect>("/detect_color");

    this->image_sub_ = image_transport::create_subscription(
      this, "input_image", std::bind(&ReadCommSequenceActionServer::image_callback, this, _1),
    ...
    while (!color_detect_client_->wait_for_service(1s)) {
      ...
      RCLCPP_INFO(
        rclcpp::get_logger("rclcpp"), "/detect_color service not available, waiting again...");
    }
    ...
    while (image_sub_.getNumPublishers() < 1) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Not recieveing images, waiting again...");
      loop_rate.sleep();
    }
...
private:
  ...
  void handle_accepted(const std::shared_ptr<GoalHandleReadCommSequence> goal_handle)
  {
    using namespace std::placeholders;
    std::thread{std::bind(&ReadCommSequenceActionServer::execute, this, _1), goal_handle}.detach();
  }
  ...
  void execute(const std::shared_ptr<GoalHandleReadCommSequence> goal_handle)
  {
    ...
    while (sequence.size() != 3) {
      ...
      RCLCPP_INFO(rclcpp::get_logger("rclcpp"), "Detected: %s", detected_color.c_str());
