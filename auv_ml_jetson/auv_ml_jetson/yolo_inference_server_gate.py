...
class GateInferenceClient(Node):
    def __init__(self):
        super().__init__("yolo_inference_client_gate")
        self.srv = self.create_service(YoloInference, "yolo_inference_gate", self.handle_inference)
        # Set the FastAPI server URL for gate inference (replace <FASTAPI_SERVER_IP> with actual IP)
        self.fastapi_url = "http://192.168.2.4:8000/inference/gate"
        self.get_logger().info(f"ROS Service Node for Gate Inference is ready. FastAPI URL: {self.fastapi_url}")
